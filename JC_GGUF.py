...
def _make_chat_handler(mmproj_path: Path, cache: bool = True):
    key = str(mmproj_path.resolve())
    if not cache:
        try:
            return Llava15ChatHandler(clip_model_path=key, clip_n_gpu_layers=0)
        except TypeError:
            os.environ.setdefault("LLAMA_CLIP_N_GPU_LAYERS", "0")
            return Llava15ChatHandler(clip_model_path=key)
...
class JC_GGUF_Models:
    def __init__(self, model: str, processing_mode: str, cache_chat: bool = True):
        try:
            ...
            chat_handler = _make_chat_handler(mmproj_path, cache=cache_chat)
            self._owned_chat = None if cache_chat else chat_handler
    ...
    def close(self):
        try:
            ...
            self.model = None
            try:
                if getattr(self, "_owned_chat", None) is not None:
                    for _m in ("close", "free", "__del__"):
                        try:
                            getattr(self._owned_chat, _m)()
                        except Exception:
                            pass
            finally:
                ...
                try:
                    if _MODEL_CACHE_LOCK:
                        with _MODEL_CACHE_LOCK:
                            ...
                            if pred is None:
                                pred = JC_GGUF_Models(GGUF_MODELS[model]["name"], processing_mode, cache_chat=True)
                                _MODEL_CACHE[strong_key] = pred
                            self.predictor = pred
                    else:
                        self.predictor = _MODEL_CACHE.get(strong_key) or JC_GGUF_Models(GGUF_MODELS[model]["name"], processing_mode, cache_chat=True)
                ...
                if (self.predictor is None or
                    ...
                    if self.predictor is not None:
                        try:
                            ...
                            pass
                        self.predictor = None
                    self.predictor = JC_GGUF_Models(GGUF_MODELS[model]["name"], processing_mode, cache_chat=(memory_management != "Clear After Run"))
                    self.current_model = model
                    self.current_processing_mode = processing_mode
